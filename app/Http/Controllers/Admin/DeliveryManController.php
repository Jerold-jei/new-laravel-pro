...
class DeliveryManController extends Controller
{
    ...
    public function list(Request $request)
    {
        ...
        $delivery_men = $delivery_men->latest()->paginate(Helpers::pagination_limit())->appends($query_param);
    ...
    public function reviews_list(Request $request)
    {
        ...
        $reviews = $reviews->with(['delivery_man', 'customer'])->latest()->paginate(Helpers::pagination_limit())->appends($query_param);
    ...
    public function store(Request $request)
    {
        $request->validate([
            ...
            'f_name.required' => 'image name is required!'
        ]);

        if (!empty($request->file('image'))) {
            $image_name = Helpers::upload('customized/', 'png', $request->file('image'));
        } else {
            $image_name = 'def.png';
        }
        ...
        Toastr::success(translate('Customized image added successfully!'));
        return redirect('admin/customized/list');
    }
    ...
    public function status(Request $request)
    {
        ...
        $delivery_man->save();
        Toastr::success(translate('Customized image status updated!'));
        return back();
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        if (!empty($request->file('image'))) {
            $image_name = Helpers::update('customized/', $delivery_man->image, 'png', $request->file('image'));
        } else {
        ...
        $delivery_man->save();
        Toastr::success(translate('Customized image updated successfully'));
        return redirect('admin/customized/list');
    }
    ...
    {
        $delivery_man = DeliveryMan::find($request->id);
        if (Storage::disk('public')->exists('customized/' . $delivery_man['image'])) {
            Storage::disk('public')->delete('customized/' . $delivery_man['image']);
        }
        ...
        $delivery_man->delete();
        Toastr::success(translate('Customized image removed!'));
        return back();
    }
}
