    ...
    <div class="page-header d-print-none">
        <div class="row align-items-center">
            <div class="col-sm mb-2 mb-sm-0">
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb breadcrumb-no-gutter">
                        ...
                        <li class="breadcrumb-item active" aria-current="page">{{\App\CentralLogics\translate('order')}} {{\App\CentralLogics\translate('details')}}</li>
                    </ol>
                </nav>
                ...
                <div class="mt-2">
                    ...
                    <a class="text-body mr-3" target="_blank" href={{route('admin.orders.generate-invoice',[$order['id']])}}>
                        <i class="tio-print mr-1"></i> {{\App\CentralLogics\translate('print')}} {{\App\CentralLogics\translate('invoice')}}
                    </a>
                    <a class="text-body mr-3" target="_blank" href={{route('admin.orders.print-address',[$order['id']])}}>
                        <i class="tio-print mr-1"></i> {{\App\CentralLogics\translate('print')}} {{\App\CentralLogics\translate('address')}}
                    </a>
                    
                    @foreach($order->details as $detail) 
                    @if($detail->product)

                    @php($data = $detail->product->category_ids)                             
                    @php($categories = json_decode($data)) 
                    @php($count = 1)
                    @foreach($categories as $category)                                
                    @php($cat_id = $category->id) 
                    @for($i = 1; $i <= $count; $i++)
                    @if(($i == $count)&&($cat_id == "4"))                      
                    <a class="text-body mr-3" target="_blank" href={{route('admin.orders.print-image',[$order['id']])}}>
                        <i class="tio-print mr-1"></i> {{\App\CentralLogics\translate('print')}} {{\App\CentralLogics\translate('customized')}}
                        {{\App\CentralLogics\translate('papad')}}
                    </a>   
                    @endif
                    @endfor
                    @endforeach
                    ...
                    <div class="hs-unfold ml-1">
                        <h5>
                            <i class="tio-shop"></i>
                            {{\App\CentralLogics\translate('branch')}} : <label class="badge badge-secondary">{{$order->branch?$order->branch->name:'Branch deleted!'}}</label>
                        </h5>
                    </div>
                    ...
                    <div class="hs-unfold float-right">
                        ...
                        <div class="dropdown">
                            ...
                            <div class="dropdown-menu text-capitalize" aria-labelledby="dropdownMenuButton">
                                <a class="dropdown-item" onclick="route_alert('{{route('admin.orders.status',['id'=>$order['id'],'order_status'=>'pending'])}}','{{translate('Change status to pending ? ')}}')" href="javascript:">{{\App\CentralLogics\translate('pending')}}</a>
                                <a class="dropdown-item" onclick="route_alert('{{route('admin.orders.status',['id'=>$order['id'],'order_status'=>'confirmed'])}}','{{translate('Change status to confirmed ? ')}}')" href="javascript:">{{\App\CentralLogics\translate('confirmed')}}</a>
                                <a class="dropdown-item" onclick="route_alert('{{route('admin.orders.status',['id'=>$order['id'],'order_status'=>'processing'])}}','{{translate('Change status to processing ? ')}}')" href="javascript:">{{\App\CentralLogics\translate('processing')}}</a>
                                <a class="dropdown-item" onclick="route_alert('{{route('admin.orders.status',['id'=>$order['id'],'order_status'=>'out_for_delivery'])}}','{{translate('Change status to out for delivery ? ')}}')" href="javascript:">{{\App\CentralLogics\translate('out_for_delivery')}}</a>
                                <a class="dropdown-item" onclick="route_alert('{{route('admin.orders.status',['id'=>$order['id'],'order_status'=>'delivered'])}}','{{translate('Change status to delivered ? ')}}')" href="javascript:">{{\App\CentralLogics\translate('delivered')}}</a>
                                <a class="dropdown-item" onclick="route_alert('{{route('admin.orders.status',['id'=>$order['id'],'order_status'=>'returned'])}}','{{translate('Change status to returned ? ')}}')" href="javascript:">{{\App\CentralLogics\translate('returned')}}</a>
                                <a class="dropdown-item" onclick="route_alert('{{route('admin.orders.status',['id'=>$order['id'],'order_status'=>'failed'])}}','{{translate('Change status to failed ? ')}}')" href="javascript:">{{\App\CentralLogics\translate('failed')}}</a>
                                <a class="dropdown-item" onclick="route_alert('{{route('admin.orders.status',['id'=>$order['id'],'order_status'=>'canceled'])}}','{{translate('Change status to canceled ? ')}}')" href="javascript:">{{\App\CentralLogics\translate('canceled')}}</a>
                            </div>
                        </div>
                        @endif
                    </div>
                    <div class="hs-unfold float-right pr-2">
                        ...
                        <div class="dropdown">
                            ...
                            <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                <a class="dropdown-item" onclick="route_alert('{{route('admin.orders.payment-status',['id'=>$order['id'],'payment_status'=>'paid'])}}','{{translate('Change status to paid ? ')}}')" href="javascript:">{{\App\CentralLogics\translate('paid')}}</a>
                                <a class="dropdown-item" onclick="route_alert('{{route('admin.orders.payment-status',['id'=>$order['id'],'payment_status'=>'unpaid'])}}','{{translate('Change status to unpaid ? ')}}')" href="javascript:">{{\App\CentralLogics\translate('unpaid')}}</a>
                            </div>
                        </div>
                        @endif
                    </div>
            ...
            <div class="col-sm-auto">
                <a class="btn btn-icon btn-sm btn-ghost-secondary rounded-circle mr-1" href="{{route('admin.orders.details',[$order['id']-1])}}" data-toggle="tooltip" data-placement="top" title="{{translate('Previous order')}}">
                    <i class="tio-arrow-backward"></i>
                </a>
                <a class="btn btn-icon btn-sm btn-ghost-secondary rounded-circle" href="{{route('admin.orders.details',[$order['id']+1])}}" data-toggle="tooltip" data-placement="top" title="{{translate('Next order')}}">
                ...
                <div class="card-body">
                    ...
                    <div class="media">
                        <div class="avatar avatar-xl mr-3">
                            <img class="img-fluid" src="{{asset('storage/app/public/product')}}/{{json_decode($detail->product['image'],true)[0]}}" onerror="this.src='{{asset('public/assets/admin/img/160x160/img2.jpg')}}'" alt="Image Description">
                        </div>
                    ...
                    <div class="row justify-content-md-end mb-3">
                        <div class="col-md-9 col-lg-8">
                            <dl class="row text-sm-right">
                                ...
                                <dd class="col-6"> - {{ Helpers::set_symbol($order['extra_discount']) }}</dd>
                                @endif
                ...
                <div class="card-body">
                    <div class="media align-items-center" href="javascript:">
                        <div class="avatar avatar-circle mr-3">
                            <img class="avatar-img" style="width: 75px" onerror="this.src='{{asset('public/assets/admin/img/160x160/img1.jpg')}}'" src="{{asset('storage/app/public/profile/'.$order->customer->image)}}" alt="Image Description">
                        </div>
                        <div class="media-body">
                    ...
                    <div class="d-flex justify-content-between align-items-center">
                        ...
                        @if(isset($address))
                        <a class="link" data-toggle="modal" data-target="#shipping-address-modal" href="javascript:">{{\App\CentralLogics\translate('edit')}}</a>
                        @endif
                    </div>
                    @if(isset($address))
                    <span class="d-block">
...
<script>
    function addDeliveryMan(id) {
        $.ajax({
            type: "GET",
            url: '{{url(' / ')}}/admin/orders/add-delivery-man/{{$order['
            id ']}}/' + id,
